...
目前共提供 **`48`** 份專屬 HTML 模板，並具備對應的即時 SVG 或高品質預覽設計。
...
- 季計畫 (`quarterly`)
- 年計畫 (`yearly`)
- 子彈筆記索引 (`bujo-index`) － 適合整理頁碼、集合與筆記總索引
- 方格 / 點陣 / 橫線頁 (`dot-grid`) － 適合草稿、手帳與計畫書寫
...
- 每日日記 (`daily-log`) － 最低成本的高頻生活紀錄模板
- 事件日記 (`event-journal`) － 用於記錄單一重要事件的經過與影響
- 回憶日記 (`memory-log`) － 回看舊經驗、整理過去與現在觀點
- 生活主題日記 (`thematic-log`) － 長期追蹤飲食、消費、健康或行程等主題
- 片段記錄 (`fragment-log`) － 快速捕捉零碎念頭與生活片段
...
- 📏 建立全站 `48` 份模板的列印驗證清單，系統化檢查 `A4 / B5 / A5 / A4裁A5` 下的裝訂線、版心、頁尾與跨頁表現
